...
# June 5th 2018
# puskyer@gmail.com
# updated for my environement
# changed MPH to KPH
# Forked from gregtinkers/carspeed.py  @  Greg Barbu
...
DISTANCE =  225  #<---- enter your distance-to-road value here
MIN_SPEED = 60  #<---- enter the minimum speed for saving images
...
#IMAGEWIDTH = 1920
#IMAGEHEIGHT = 1080
# for 5MP  Wide Angle Fish Eye 139 defree - does not work black screen
#IMAGEWIDTH = 2592
#IMAGEHEIGHT = 1944
RESOLUTION = [IMAGEWIDTH,IMAGEHEIGHT]
#FOV = 53.5   #<---- Field of view
FOV = 69.9   #<---- Angle  of view for  Wide Angle Fish eye 130 Deg 5MP cam from wish.com
# for reguler RaspPi Cam and 1080p
#FPS = 30
# for 720p
FPS = 60
...
for frame in camera.capture_continuous(rawCapture, format="bgr", use_video_port=True):
    ...
    if motion_found:
        if state == WAITING:
            ...
            print(text_on_image)
            print("x-chg    Secs      KPH  x-pos width")
        else:
            ...
            if state == TRACKING:
                ...
                print("{0:4d}  {1:7.2f}  {2:7.0f}   {3:4d}  {4:4d}".format(abs_chg,secs,kph,x,w))
                ...
                if ((x <= 2) and (direction == RIGHT_TO_LEFT)) \
                    ...
                    if (last_kph > MIN_SPEED):    # save the image
                        # timestamp the image
                        cv2.putText(image, datetime.datetime.now().strftime("%A %d %B %Y %I:%M:%S%p"),
                            (10, image.shape[0] - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 255, 0), 1)
                        # write the speed: first get the size of the text
                        size, base = cv2.getTextSize( "%.0f kph" % last_kph, cv2.FONT_HERSHEY_SIMPLEX, 2, 3)
                        # then center it horizontally on the image
                        cntr_x = int((IMAGEWIDTH - size[0]) / 2) 
                        cv2.putText(image, "%.0f kph" % last_kph,
                        ...
                        #imageFilename = "car_at_%02.0f" % last_kph + "_" + datetime.datetime.now().strftime("%Y%m%d_%H%M%S") + ".jpg"

                        cv2.imwrite(imageFilename,image)
                        if SAVE_CSV:
                            cap_time = datetime.datetime.now()
                            record_speed(cap_time.strftime("%Y.%m.%d")+','+cap_time.strftime('%A')+','+\
                               cap_time.strftime('%H%M')+','+("%.0f" % last_kph) + ','+imageFilename)
                    state = SAVING
